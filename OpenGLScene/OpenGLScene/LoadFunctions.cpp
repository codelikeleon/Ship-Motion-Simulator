...
GLuint LoadBMPSkybox( vector<const GLchar * > skyboxFaces ) {
    ...
    for (GLuint i = 0; i < skyboxFaces.size(); i++) {
        ...
        FILE * file = fopen(skyboxFaces[i], "rb");
        if (!file ) {
...
/*
 * Simple OBJ file loader that will not work with more complex OBJ files
 *
 * This loader:
...
bool LoadOBJ(  const char* obj_file_path
    ...
    while( 1 ){
        ...
        if ( strcmp( lineHeader, "v" ) == 0 ){
            ...
            if (matches != 9){
                fprintf(stderr, "File can't be read by the OBJ parser. Try an OBJ export with different options : %s\n", obj_file_path);
                return false;
            }
